<template>
    <section class="buyers-ranking">
        ...
        <p class="subtitle">Top 12 des acheteurs classés par montant total des contrats conclus au cours de <span class="highlight">71 derniers mois</span>. Survolez les noms les acheteurs pour les afficher en entier.</p>
        <div class="tabs">
            <button v-for="tab in tabs" :key="tab.key" :class="['tab', { active: tab.key === selectedTab }]" @click="selectedTab = tab.key">
                {{ tab.label }}
            </button>
        </div>
        <div class="buyers-chart-row">
            <div class="buyers-list">
                <div v-for="(buyer, i) in sortedBuyersData[selectedTab]" :key="buyer.name" class="buyer-item">
            ...
            <div class="buyers-bar-chart">
                <Chart type="bar" :data="barChartData" :options="barChartOptions" class="buyers-chart" />
            </div>
        </div>
        <a href="#" class="buyers-link"> <i class="pi pi-list"></i> Liste complète des organismes acheteurs </a>
    </section>
</template>
...
const barColors = ['#f6d7a7', '#f6b26b', '#b6d7a8', '#a2c4c9', '#b4a7d6', '#a4c2f4', '#b7b7b7', '#ffd966', '#d9ead3', '#cfe2f3', '#f4cccc', '#ead1dc'];
